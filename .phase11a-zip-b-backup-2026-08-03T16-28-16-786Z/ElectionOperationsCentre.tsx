...
export default function ElectionOperationsCentre() {
  ...
  return (
    <section className="space-y-4">
      ...
      {activePanel === "agents" && (
        <article className="border border-border bg-card p-4">
          <p className="font-mono text-[10px] tracking-widest">AGENT OPERATIONS</p>
      ...
      {activePanel === "vehicles" && (
        <article className="border border-border bg-card p-4">
          <p className="font-mono text-[10px] tracking-widest">VEHICLE DEPLOYMENT</p>
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {(data?.vehicles ?? []).map((vehicle) => (
              <div key={vehicle.id} className="border border-border p-3">
      ...
      {activePanel === "observers" && (
        <article className="border border-border bg-card p-4">
          <p className="font-mono text-[10px] tracking-widest">OBSERVER MANAGEMENT</p>
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {(data?.observers ?? []).map((observer) => (
              <div key={observer.id} className="border border-border p-3">
      ...
      {activePanel === "escalations" && (
        <article className="border border-border bg-card p-4">
          <p className="font-mono text-[10px] tracking-widest">INCIDENT ESCALATION MATRIX</p>
          <div className="mt-4 space-y-2">
            {(data?.escalations ?? []).map((incident) => (
              <div
                key={incident.id}
                className="grid gap-3 border border-border p-3 md:grid-cols-[1fr_auto]"
              >
                <div>
