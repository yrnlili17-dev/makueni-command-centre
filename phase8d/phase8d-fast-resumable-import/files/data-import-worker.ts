...
async function processBatch(
  ...
  const statuses = options.importWarnings
    ? ["valid", "warning"]
    : ["valid"];
  ...
  if (duplicates.length) {
    ...
    await db.execute(sql`
      ...
      WHERE id = ANY(${duplicateIds}::bigint[])
    `);
  }
  ...
  const existingResult = await db.execute(sql`
    ...
    WHERE
      (
        cardinality(${nationalIds}::text[]) > 0
        AND national_id = ANY(${nationalIds}::text[])
      )
      OR (
        cardinality(${phones}::text[]) > 0
