...
export async function ensurePhase3Tables(): Promise<void> {
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS "approval_requests" (
      ...
      "requested_by_id" integer NOT NULL, "requested_by_email" text NOT NULL, "reviewed_by_id" integer,
      "reviewed_by_email" text, "review_comment" text, "expires_at" timestamp NOT NULL, "reviewed_at" timestamp,
      "executed_at" timestamp, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now()
    )
  `);
  await db.execute(sql`
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS "document_folders" (
      "id" serial PRIMARY KEY, "name" text NOT NULL, "description" text, "parent_id" integer,
      "visibility" text NOT NULL DEFAULT 'internal', "created_by_id" integer NOT NULL, "created_by_email" text NOT NULL,
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS "documents" (
      ...
      "storage_type" text NOT NULL DEFAULT 'local', "storage_url" text, "category" text NOT NULL DEFAULT 'general',
      "tags" jsonb NOT NULL DEFAULT '[]', "version" integer NOT NULL DEFAULT 1, "checksum" text, "status" text NOT NULL DEFAULT 'active',
      "requires_approval" boolean NOT NULL DEFAULT false, "approval_request_id" integer, "uploaded_by_id" integer NOT NULL,
      "uploaded_by_email" text NOT NULL, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now()
    )
  `);
  await db.execute(sql`
...
const MODULES = [
  ...
  "finance", "election-day", "credentials", "admin", "approvals", "documents", "geography",
];
