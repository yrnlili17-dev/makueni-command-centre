  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "documents" && <div className="space-y-4">
        ...
        {showFolder && <form onSubmit={createFolder} className="flex gap-2 border border-border bg-card p-3"><input autoFocus value={folderName} onChange={(e) => setFolderName(e.target.value)} placeholder="Folder name" className="flex-1 border border-border bg-background px-3 py-2" /><button type="submit" className="bg-primary px-4 font-mono text-xs text-primary-foreground">CREATE</button><button type="button" onClick={() => setShowFolder(false)} className="border border-border px-4 font-mono text-xs">CANCEL</button></form>}
