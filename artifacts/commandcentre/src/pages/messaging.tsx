...
import {
  ...
  useDeleteCampaign,
  useSendCampaign,
  useGetMessagingStats,
  useListSegments,
  getListCampaignsQueryKey,
  getGetMessagingStatsQueryKey,
...
function ChannelBadge({ channel }: { channel: string }) {
  ...
  return (
    <span
      className={`font-mono text-[10px] border px-1.5 py-0.5 ${map[channel] ?? "border-border text-muted-foreground"}`}
    >
      {channel.toUpperCase()}
    </span>
  );
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`font-mono text-[10px] border px-1.5 py-0.5 ${map[status] ?? "border-border text-muted-foreground"}`}
    >
      [ {status.toUpperCase()} ]
    </span>
  );
    ...
    <div className="bg-card border border-border">
        ...
        <div className="flex items-center gap-3">
          ...
          {provider?.isActive && (
            <span className="font-mono text-[9px] text-green-400 border border-green-400/30 px-1.5 py-0.5">
              ◆{" "}
              {SMS_PROVIDERS.find((p) => p.value === provider.provider)
      ...
      {open && (
        <div className="border-t border-border p-4">
          {saved && (
            <div className="mb-3 bg-green-500/10 border border-green-500/30 text-green-400 font-mono text-xs p-2">
              ✓ SMS PROVIDER SAVED
            </div>
          )}
          <form onSubmit={save} className="space-y-3">
            ...
            {(providerVal === "africastalking" || providerVal === "custom") && (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2">
                  <FieldInput
                    label="SENDER ID (SHORTCODE / ALPHANUMERIC)"
                    placeholder="e.g. MULE or 40100"
                    value={form.senderId}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, senderId: e.target.value }))
                    }
                  />
                  <p className="font-mono text-[9px] text-muted-foreground mt-1">
                    Africa's Talking registered sender ID. Leave blank to use
                    default shared shortcode.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-secondary border border-border p-3 font-mono text-[9px] text-muted-foreground space-y-0.5">
              {providerVal === "africastalking" && (
                <>
                  ...
                  <div>
                    2. Create an app → copy the API key from the dashboard
                  </div>
                  <div>
                    3. Use "sandbox" as username during testing, or your real
                    username in production
                  </div>
                  <div>
                    4. Request a Sender ID (shortcode/alphanumeric) through the
                    AT dashboard
                  </div>
                </>
              )}
              {providerVal === "twilio" && (
                <>
                  ...
                  <div>
                    3. Account SID and Auth Token are on the Console dashboard
                  </div>
                </>
              )}
              {providerVal === "infobip" && (
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-2 bg-primary text-white font-mono text-[10px] tracking-widest px-4 py-2 hover:bg-primary/90 disabled:opacity-50"
              >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {provider?.isActive && (
            <span className="font-mono text-[9px] text-green-400 border border-green-400/30 px-1.5 py-0.5">
              ◆{" "}
              {WA_PROVIDERS.find((p) => p.value === provider.provider)?.label ??
                provider.provider}
            </span>
          )}
        </div>
          ...
          <form onSubmit={save} className="space-y-3">
            ...
            {providerVal === "meta" && (
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  />
                  <p className="font-mono text-[9px] text-muted-foreground mt-1">
                    Generate in Meta Business Suite → System Users → Generate
                    Token
                  </p>
                </div>
                <FieldInput
            ...
            {providerVal === "twilio_wa" && (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2">
                  <FieldInput
                    label="WHATSAPP FROM NUMBER (with whatsapp: prefix)"
            ...
            <div className="bg-secondary border border-border p-3 font-mono text-[9px] text-muted-foreground space-y-0.5">
              {providerVal === "meta" && (
                <>
                  ...
                  <div>
                    4. Create a System User and generate a Permanent Access
                    Token
                  </div>
                  <div>
                    5. Phone Number ID and WABA ID are in the WhatsApp Manager
                  </div>
                </>
              )}
              {providerVal === "africastalking_wa" && (
              ...
              {providerVal === "twilio_wa" && (
                <>
                  ...
                  <div>
                    2. Use Twilio sandbox for testing, or apply for a dedicated
                    number
                  </div>
                </>
              )}
        ...
        <div className="flex items-center gap-3">
          ...
          {provider?.isActive && (
            <span className="font-mono text-[9px] text-purple-400 border border-purple-400/30 px-1.5 py-0.5">
              ◆{" "}
              {EMAIL_PROVIDERS.find((p) => p.value === provider.provider)
                ?.label ?? provider.provider}
            </span>
          )}
        </div>
          ...
          <form onSubmit={save} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <FieldInput
                    ...
                    type="password"
                    placeholder={
                      providerVal === "sendgrid" ? "SG.xxxxxxxx..." : "AKIA..."
                    }
                    value={form.apiKey}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, apiKey: e.target.value }))
                ...
                <FieldInput
                  ...
                  required
                  type="email"
                  placeholder="campaign@kaloki2027.ke"
                ...
                <FieldInput
                  label="FROM NAME"
                  placeholder="Prof. Philip Kaloki Campaign"
                  value={form.fromName}
                  onChange={(e) =>
            ...
            {providerVal === "mailchimp" && (
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <FieldInput
                    label="MANDRILL / MAILCHIMP API KEY"
                    required
                    type="password"
                    placeholder="md-xxxxxxxxxxxxxxxxxxxxxxxx"
                    value={form.apiKey}
                    onChange={(e) =>
                      setForm((p) => ({ ...p, apiKey: e.target.value }))
                    }
            ...
            {providerVal === "smtp" && (
              <div className="grid grid-cols-2 gap-3">
                <FieldInput
                  ...
                  value={form.smtpHost}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, smtpHost: e.target.value }))
                  }
                />
                <FieldInput
                  label="PORT"
                  ...
                  value={form.smtpPort}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, smtpPort: e.target.value }))
                  }
                />
                <FieldInput
                  label="SMTP USERNAME"
                  ...
                  value={form.smtpUser}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, smtpUser: e.target.value }))
                  }
                />
                <FieldInput
                  label="SMTP PASSWORD"
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="font-mono text-[10px] text-muted-foreground tracking-widest">
          ◆ {channelLabel.toUpperCase()} CAMPAIGNS
        </div>
        <button
          onClick={() => setShowAdd((s) => !s)}
          className="flex items-center gap-1.5 bg-primary text-white font-mono text-[10px] tracking-widest px-3 py-2 hover:bg-primary/90"
        >
        ...
        <div className="bg-card border border-primary/40 p-4">
          <div className="flex justify-between items-center mb-4">
            <div className="font-mono text-[10px] tracking-widest">
              NEW {channelLabel.toUpperCase()} TRANSMISSION
            </div>
            <button
              onClick={() => {
                setShowAdd(false);
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 space-y-1">
              <div className="flex justify-between">
                ...
                {charLimit && (
                  <span
                    className={`font-mono text-[10px] ${(form.messageBody?.length ?? 0) > charLimit ? "text-red-400" : "text-muted-foreground"}`}
                  >
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, messageBody: e.target.value }))
                }
                ...
                placeholder={
                  channel === "email"
                    ...
                    : channel === "whatsapp"
                      ? "WhatsApp message (supports *bold*, _italic_, ~strikethrough~)..."
                      : "SMS message (max 160 chars for 1 part)..."
                }
            ...
            <div className="flex items-end justify-end gap-2">
              <button
                ...
                className="flex items-center gap-1.5 border border-border px-4 py-2 font-mono text-[10px] hover:bg-secondary"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-primary text-white px-4 py-2 font-mono text-[10px] hover:bg-primary/90 disabled:opacity-50"
              >
          ...
          <thead>
            <tr className="border-b border-border bg-secondary/30">
              {[
                ...
                <th
                  key={h}
                  className="px-4 py-2 text-left font-mono text-[10px] text-muted-foreground tracking-widest"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-border">
            {isLoading ? (
              <tr>
                <td
                  colSpan={7}
                  className="px-4 py-8 text-center font-mono text-xs text-muted-foreground animate-pulse"
                >
              ...
              <tr>
                ...
                >
                  [ NO_{channel.toUpperCase()}_CAMPAIGNS ]
                </td>
              </tr>
            ) : (
              campaigns.map((c) => (
...
const TABS: { id: Channel; label: string; color: string; desc: string }[] = [
  {
    ...
    desc: "Short message service — bulk SMS broadcast",
  },
  {
    id: "whatsapp",
    label: "WHATSAPP",
    color: "text-green-400 border-green-400",
    desc: "WhatsApp Business API messaging",
  },
  {
    id: "email",
    label: "EMAIL",
...
export default function Messaging() {
  ...
  return (
    <div className="space-y-5">
        ...
        {stats && (
          <div className="flex gap-3">
            {[
              ...
              <div
                key={s.label}
                className="bg-card border border-border px-4 py-2 text-center min-w-[80px]"
              >
      ...
      {channelStats && (
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: "SENT",
              value: channelStats.sent.toLocaleString(),
              color: "",
            },
            {
              label: "DELIVERED",
              value: channelStats.delivered.toLocaleString(),
              color: "text-green-400",
            },
            {
              label: "OPEN RATE",
              value: `${(channelStats.openRate * 100).toFixed(1)}%`,
              color: "text-blue-400",
            },
          ].map((s) => (
      ...
      {tab === "sms" && (
        <SmsConfig provider={smsProvider} onSaved={loadProviders} />
      )}
      {tab === "whatsapp" && (
        <WhatsAppConfig provider={waProvider} onSaved={loadProviders} />
      )}
      {tab === "email" && (
        <EmailConfig provider={emailProvider} onSaved={loadProviders} />
      )}
    </div>
  );
}
