...
import {
  useListCanvassSessions,
  useCreateCanvassSession,
  useGetFieldCoverage,
  useListCanvassVisits,
  useLogCanvassVisit,
  getListCanvassSessionsQueryKey,
  getGetFieldCoverageQueryKey,
  getListCanvassVisitsQueryKey,
...
const PLAYBOOK = [
  ...
  {
    key: "church-outreach",
    label: "FAITH COMMUNITY OUTREACH",
    icon: Church,
    color: "#8B5CF6",
  ...
  {
    key: "youth-mobilization",
    label: "BODABODA MOBILIZATION",
    icon: Zap,
    color: "#EAB308",
  ...
  {
    key: "kol-engagement",
    label: "OPINION LEADER ENGAGEMENT",
    icon: Star,
    color: "#10B981",
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`font-mono text-[9px] border px-1.5 py-0.5 ${STATUS_CLS[status] ?? "text-muted-foreground border-border"}`}
    >
      [ {status.toUpperCase()} ]
    </span>
  );
...
export default function FieldOps() {
  ...
  const { data: visits } = useListCanvassVisits({
  ...
  const createSession = useCreateCanvassSession({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListCanvassSessionsQueryKey() });
        setShowAdd(false);
        setSessionForm({ type: "baraza", ward: WARDS[0] });
      },
    },
  });
  const logVisit = useLogCanvassVisit({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListCanvassVisitsQueryKey() });
        qc.invalidateQueries({ queryKey: getGetFieldCoverageQueryKey() });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[10px] font-mono text-muted-foreground mt-1 tracking-widest">
            GROUND GAME COMMAND · 165 POLLING STATIONS · MAKUENI COUNTY
          </p>
        </div>
          ...
          {[
            {
              ...
              value: fieldStats.totalSessions,
              color: "text-foreground",
            },
            {
              label: "ACTIVE NOW",
              value: fieldStats.active,
              color: "text-green-400",
            },
            {
              label: "CONTACTS MADE",
              value: fieldStats.totalContacts.toLocaleString(),
              color: "text-foreground",
            },
            {
              label: "SUPPORTERS ID'd",
              value: fieldStats.supporters.toLocaleString(),
              color: "text-primary",
            },
            {
              label: "UNDECIDED",
              value: fieldStats.undecided.toLocaleString(),
              color: "text-yellow-400",
            },
            {
              label: "WARDS ACTIVE",
              value: `${fieldStats.wards}/5`,
              color: "text-foreground",
            },
        ...
        <div className="bg-card border border-border relative overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-border flex items-center gap-2">
            ...
            <span className="font-mono text-[9px] text-muted-foreground border border-border px-1.5 py-0.5">
              8 PROVEN ELECTION-WINNING TACTICS
            </span>
          </div>
          <div className="grid grid-cols-4 divide-x divide-y divide-border">
            {PLAYBOOK.map(
              ({ key, label, icon: Icon, color, tagline, why, tip }) => (
                <div key={key} className="p-4 space-y-2">
                  ...
                  >
                    <p className="font-mono text-[8px] text-muted-foreground/80 italic">
                      TIP: {tip}
                    </p>
                  </div>
                </div>
        ...
        <div className="bg-card border border-primary/50 p-4 relative overflow-hidden">
            ...
            onSubmit={(e) => {
              ...
              createSession.mutate({
                data: {
                  ...
                  ward,
                  date,
                  doorsTarget: Number(doorsTarget ?? 0),
                  assignedVolunteers: Number(assignedVolunteers ?? 0),
                  notes,
                } as CanvassSessionInput,
              });
            }}
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <select
                required
                value={sessionForm.ward ?? ""}
                onChange={(e) =>
                  setSessionForm((p) => ({ ...p, ward: e.target.value }))
                }
                className="w-full bg-secondary border border-border px-3 py-2 font-mono text-xs focus:outline-none focus:border-primary"
              >
                {WARDS.map((w) => (
            ...
            <div className="col-span-2 space-y-1">
              ...
              <input
                ...
                placeholder="e.g. Wote Market Friday Baraza"
                className="w-full bg-secondary border border-border px-3 py-2 font-mono text-xs focus:outline-none focus:border-primary"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                type="number"
                value={sessionForm.assignedVolunteers ?? ""}
                onChange={(e) =>
                  setSessionForm((p) => ({
                    ...p,
                    assignedVolunteers: e.target.value,
                  }))
                }
                className="w-full bg-secondary border border-border px-3 py-2 font-mono text-xs focus:outline-none focus:border-primary"
              />
            ...
            <div className="space-y-1">
              ...
              <input
                type="number"
                value={sessionForm.doorsTarget ?? ""}
                onChange={(e) =>
                  setSessionForm((p) => ({ ...p, doorsTarget: e.target.value }))
                }
                className="w-full bg-secondary border border-border px-3 py-2 font-mono text-xs focus:outline-none focus:border-primary"
              />
            </div>
            <div className="col-span-2 space-y-1">
              ...
              <input
                ...
                placeholder="Objectives, location, intelligence notes…"
                className="w-full bg-secondary border border-border px-3 py-2 font-mono text-xs focus:outline-none focus:border-primary"
              />
            </div>
      ...
      <div className="flex gap-0 flex-wrap border-b border-border">
        {SESSION_TYPES.map((t) => {
          const count =
            t.key === "all"
              ? (sessions ?? []).length
              : (sessions ?? []).filter((s) => s.type === t.key).length;
          return (
            <button
              key={t.key}
            ...
            >
              ...
              {count > 0 && (
                <span
                  className={`ml-1.5 text-[8px] px-1 py-0.5 ${typeFilter === t.key ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"}`}
                >
                  {count}
                </span>
              )}
      ...
      <div
        className={`grid gap-4 ${selectedSession ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}
      >
        {/* Sessions */}
        <div>
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-card border border-border h-28 animate-pulse"
                />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            ...
            <div
              className={`grid gap-2 ${selectedSession ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2 xl:grid-cols-3"}`}
            >
              {filtered.map((s) => {
                ...
                const progress =
                  s.doorsTarget > 0
                    ? Math.min((s.doorsCompleted / s.doorsTarget) * 100, 100)
                    : 0;
                return (
                  <div
                    key={s.id}
                    onClick={() =>
                      setSelectedSession(s.id === selectedSession ? null : s.id)
                    }
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <ChevronRight
                        className={`w-3.5 h-3.5 text-muted-foreground shrink-0 transition-transform ${s.id === selectedSession ? "rotate-90 text-primary" : ""}`}
                      />
                    </div>
                    <div className="flex items-center gap-3 mb-2">
                      {s.assignedVolunteers > 0 && (
                        <span className="font-mono text-[9px] text-muted-foreground flex items-center gap-1">
                          ...
                          {s.assignedVolunteers} VOL
                        </span>
                      )}
                      {s.doorsTarget > 0 && (
                        <span className="font-mono text-[9px] text-muted-foreground flex items-center gap-1">
                          <Target className="w-2.5 h-2.5" />
                          {s.doorsCompleted}/{s.doorsTarget}
                        </span>
                      )}
                    </div>
                    {s.doorsTarget > 0 && (
                    ...
                    {s.notes && (
                      <p className="font-mono text-[8px] text-muted-foreground/70 mt-2 line-clamp-2 italic leading-relaxed">
                        {s.notes}
                      </p>
                    )}
                  </div>
          ...
          <div className="space-y-0">
            <div className="bg-card border border-primary/30 relative overflow-hidden">
              ...
              <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                <div>
                  <p className="font-mono text-[10px] text-primary tracking-widest font-bold">
                    {selectedSessionData.name}
                  </p>
                  <p className="font-mono text-[8px] text-muted-foreground mt-0.5">
                    {selectedSessionData.ward} · {selectedSessionData.date}
                  </p>
                </div>
                <div className="flex gap-2">
                  <button
              ...
              {visits && visits.length > 0 && (
                <div className="grid grid-cols-5 divide-x divide-border border-b border-border">
                  {[
                    {
                      label: "SUPPORT",
                      count: visits.filter((v) => v.outcome === "support")
                        .length,
                      color: "text-green-400",
                    },
                    {
                      label: "UNDECIDED",
                      count: visits.filter((v) => v.outcome === "undecided")
                        .length,
                      color: "text-yellow-400",
                    },
                    {
                      label: "OPPOSE",
                      count: visits.filter((v) => v.outcome === "oppose")
                        .length,
                      color: "text-red-400",
                    },
                    {
                      label: "NOT HOME",
                      count: visits.filter((v) => v.outcome === "not_home")
                        .length,
                      color: "text-muted-foreground",
                    },
                    {
                      label: "REFUSED",
                      count: visits.filter((v) => v.outcome === "refused")
                        .length,
                    ...
                    <div key={label} className="px-3 py-2 text-center">
                      ...
                      <p className="font-mono text-[7px] text-muted-foreground tracking-widest">
                        {label}
                      </p>
                    </div>
                  ))}
                  ...
                  >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setVisitForm((p) => ({
                            ...p,
                            outcome: e.target
                              .value as CanvassVisitInput["outcome"],
                          }))
                        }
                        className="w-full bg-card border border-border px-2 py-1.5 font-mono text-xs focus:outline-none focus:border-primary"
                      >
                    ...
                    <div className="col-span-2 space-y-1">
                      ...
                      <input
                        ...
                        placeholder="Concerns raised, commitments made, follow-up needed…"
                        className="w-full bg-card border border-border px-2 py-1.5 font-mono text-xs focus:outline-none focus:border-primary"
                      />
                    </div>
                    <div className="col-span-2 flex gap-2 justify-end">
                      <button
                        ...
                        className="border border-border px-3 py-1.5 font-mono text-[9px] hover:bg-secondary"
                      >
                        ABORT
                      </button>
                      <button
                        ...
                        className="bg-primary text-primary-foreground px-3 py-1.5 font-mono text-[9px] hover:bg-primary/90 disabled:opacity-60"
                      >
                        LOG
                      </button>
                    </div>
                ...
                {!visits || visits.length === 0 ? (
                  ...
                  visits.map((v) => {
                    const oc: Record<string, string> = {
                      ...
                      oppose: "text-red-400",
                      undecided: "text-yellow-400",
                      not_home: "text-muted-foreground",
                      refused: "text-orange-400",
                    };
                    return (
                      <div
                        key={v.id}
                      ...
                      >
                        ...
                        <span
                          className={`font-mono text-[9px] shrink-0 ${oc[v.outcome] ?? "text-muted-foreground"}`}
                        >
                ...
                <div key={ward} className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4 font-mono text-[9px]">
                      <span className="text-green-400">
                        {(w.supportCount || 0).toLocaleString()} SUP
                      </span>
                      <span className="text-yellow-400">
                        {(w.undecidedCount || 0).toLocaleString()} UND
                      </span>
                      <span className="text-red-400">
                        {(w.opposeCount || 0).toLocaleString()} OPP
                      </span>
                      <span className="text-muted-foreground">
                        {w.doorsKnocked.toLocaleString()} /{" "}
                  ...
                  <div className="h-2 bg-secondary flex overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(((w.supportCount || 0) / voters) * 100, 100)}%`,
                      }}
                    />
                    <div
                      className="h-full bg-yellow-400/70 transition-all"
                      style={{
                        width: `${Math.min(((w.undecidedCount || 0) / voters) * 100, 100)}%`,
                      }}
                    />
                    <div
                      className="h-full bg-red-400/50 transition-all"
                      style={{
                        width: `${Math.min(((w.opposeCount || 0) / voters) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
