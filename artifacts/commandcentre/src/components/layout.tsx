...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground font-sans">
      <aside className="w-64 shrink-0 border-r border-border bg-card/95 flex flex-col">
        <div className="p-6 flex items-center gap-3 border-b border-border">
          <img src={brandIcon} alt="CommandCentre OS" className="w-8 h-8 object-contain" />
          <div className="flex flex-col">
            <span className="font-bold tracking-widest text-sm">MAKUENI COMMAND CENTRE</span>
      ...
      <main className="flex min-w-0 flex-1 flex-col h-screen overflow-hidden">
        <header className="min-h-14 border-b border-border flex flex-wrap items-center gap-3 px-4 py-3 justify-between shrink-0 bg-card/80 backdrop-blur md:px-6">
          <div className="flex items-center gap-4">
            <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
            <span className="font-mono text-[10px] text-muted-foreground tracking-widest sm:text-xs">MAKUENI COUNTY · CAMPAIGN COMMAND CENTRE · SECURE OPERATIONS</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="hidden font-mono text-[10px] text-muted-foreground xl:inline">TIME: {new Date().toISOString()}</span>
